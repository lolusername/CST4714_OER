...
def main() -> int:
    ...
    book_files = sorted(str(p.relative_to(ROOT)) for p in (ROOT / "textbook").rglob("*") if p.is_file())
    check(book_files == ["textbook/Operating_Cloud_Databases.pdf"], "Textbook must be one PDF only")
    check((ROOT / book_files[0]).read_bytes().startswith(b"%PDF-"), "Invalid PDF header")
    ...
    weeks = sorted((ROOT / "weeks").glob("week_[0-9][0-9]"))
    ...
    data = json.loads((ROOT / "datasets/cisa_kev_sample/kev_sample.json").read_text())
    check(data["count"] == len(data["vulnerabilities"]) == 75, "CISA sample count must be 75")
    with (ROOT / "datasets/cisa_kev_sample/kev_sample.csv").open(newline="") as stream:
    ...
    check(len(list((ROOT / "assessments").glob("*.md"))) == 6, "Expected six assessment resources")
    for name in ("midterm_project.md", "final_project.md"):
        check((ROOT / "assignments" / name).is_file(), f"Missing project: {name}")
